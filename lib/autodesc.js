...
// LANGUAGE-SPECIFIC DATA AND METHODS
    ...
    modifierStrings : {
        en : {
            'nationality' : {'Ecuador':'Ecuadorian','Ghana':'Ghanaian','Russia':'Russian','Argentina':'Argentine','Australia':'Australian',
                ...
                'United States of America':'American','Sweden':'Swedish','Egypt':'Egyptian','Vatican City':''}
        } ,
        de : {
    ...
    getNationality : function ( claims , opt ) {
        ...
        tmp.forEach( function ( v ) {
            var s = self.getNationalityFromCountry ( v.qLabel , claims , { lang:opt.lang , not_last:(index+1!=tmp.length) } ) ;
    ...
    loadItem : function ( q , opt, callback ) {
        var self = this;
        q = q.toUpperCase();
