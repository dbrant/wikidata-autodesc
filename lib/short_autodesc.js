...
// LANGUAGE-SPECIFIC DATA AND METHODS
    ...
    getCountryOfOrigin : function ( item_labels , claims , opt ) {
        var self = this;
        var h2 = '' ;
        var tmp = item_labels[properties.P.country_of_origin]||[] ;
        $.each ( tmp , function ( k , v ) {
            var v2 = self.splitLink ( v ) ;
            var s = self.getNationalityFromCountry ( v2[2] , claims , { lang:opt.lang , not_last:(k+1!=tmp.length) } ) ;
            if ( k == 0 ) h2 = v2[1]+s+v2[3] ;
    ...
    describePerson : function ( q , claims , opt ) {
        ...
        self.labelItems ( load_items , function ( item_labels ) {
            ...
            // Occupation
            var ol = h.length ;
            self.add2desc ( h , item_labels , [ properties.P.occupation ] , { hints:{is_male:is_male,is_female:is_female,occupation:true}, o:opt, max_count:2 } ) ;
            if ( h.length == ol ) h.push ( self.txt('person',opt.lang) ) ;

            // Office
            self.add2desc ( h , item_labels , [ properties.P.position_held ] , { hints:{is_male:is_male,is_female:is_female,office:true}, prefix:',', o:opt, max_count:2 } ) ;
    ...
    describeTaxon : function ( q , claims , opt ) {
        ...
        self.labelItems ( load_items , function ( item_labels ) {
            ...
            self.add2desc ( h , item_labels , [ properties.P.subclass_of, properties.P.instance_of, properties.P.is_a, properties.P.taxon_rank ] , { o:opt } ) ;
    ...
    isFilm : function ( claims ) {
        var self = this;
        return ( self.hasPQ( claims , properties.P.instance_of, properties.Q.film) || self.hasPQ( claims , properties.P.instance_of, properties.Q.television_program) || self.hasPQ( claims , properties.P.instance_of, properties.Q.television_series) );
    },
    ...
    describeFilm : function ( q , claims , opt ) {
        ...
        var load_items = [] ;
        self.addItemsFromClaims ( claims , properties.P.instance_of , load_items );
        self.addItemsFromClaims ( claims , properties.P.publication_date , load_items );
        self.addItemsFromClaims ( claims , properties.P.director , load_items );
        self.addItemsFromClaims ( claims , properties.P.producer , load_items );
        self.addItemsFromClaims ( claims , properties.P.genre , load_items );
        self.addItemsFromClaims ( claims , properties.P.country_of_origin , load_items );
        self.addItemsFromClaims ( claims , properties.P.cast_member , load_items );

        self.labelItems ( load_items , function ( item_labels ) {
            var h = [];
            var h2;

            var pubdate = self.getYear ( claims , properties.P.publication_date , opt.lang ) ;
            if ( pubdate != '' ) h.push ( pubdate ) ;

            h2 = self.getCountryOfOrigin(item_labels, claims, opt);
            if ( h2 != '' ) h.push ( h2 ) ;

            if (self.hasP(claims, properties.P.genre) && self.hasPQ( claims , properties.P.instance_of, properties.Q.film)) {
                self.add2desc ( h , item_labels , [ properties.P.genre ] , { o:opt, max_count:1 } ) ;
            } else {
                self.add2desc ( h , item_labels , [ properties.P.instance_of ] , { o:opt, max_count:1 } ) ;
            }

            self.add2desc ( h , item_labels , [ properties.P.director ] , { txt_key:'directed by', o:opt } ) ;
            self.add2desc ( h , item_labels , [ properties.P.cast_member ] , { prefix:',', txt_key:'starring', o:opt, max_count:2 } ) ;

            // Fallback
            if ( h.length == 0 ) h.push ( self.txt('film', opt.lang) ) ;
            self.setTarget ( opt ,  self.ucFirst ( h.join ( ' ' ) ) , q ) ;
        }, opt ) ;
    },
