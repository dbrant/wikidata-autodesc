...
// LANGUAGE-SPECIFIC DATA AND METHODS
    ...
    modifyWord : function ( word , hints , lang ) {
        if ( lang == 'en' ) {
            if ( hints.is_female ) {
                word = word.toLowerCase().replace('actor', 'actress');
            } else if ( hints.is_male ) {
                word = word.toLowerCase().replace('actress', 'actor');
            }
        } else if ( lang == 'fr' ) {
            if ( hints.is_female ) {
    ...
    getNationality : function ( claims , opt ) {
        ...
        $.each ( tmp , function ( k , v ) {
            var s = self.getNationalityFromCountry ( v.qLabel , claims , { lang:opt.lang , not_last:(k+1!=tmp.length) } ) ;
            if ( k == 0 ) h2 = s;
    ...
    add2desc : function ( h , claims , props , opt ) {
        ...
        props.forEach( function ( prop ) {
            (claims[self.p_prefix+prop]||[]).forEach( function( claim ) {
                if (undefined !== opt.max_count && count >= opt.max_count) {
                    return;
                }
                count++;
                finalLabel = '';
    ...
    assignQValuesToP : function ( claims ) {
        for (var prop in claims) {
            ...
            claims[prop].forEach(function (v) {
                ...
                v.qId = v.mainsnak.datavalue.value['numeric-id'];
                v.qLabel = '';
            });
        }
    } ,
    ...
    labelItems : function ( claims , callback , opt ) {
        ...
        claims.forEach(function (claim) {
            if (undefined === claim.qId) {
                self.logd("Warning: claim without Q-id");
            } else {
                if (claim.qId != 0) {
                    itemsToFetch.push(self.q_prefix+claim.qId);
                }
            }
        });

        self.logd("Fetching batch of items: " + JSON.stringify(itemsToFetch));
        self.wd.getItemBatch(itemsToFetch, function () {
