...
var reasonator = {
    ...
    init : function () {
        ...
        $.each ( ['father','mother','child','brother','sister','spouse','uncle','aunt','stepfather','stepmother','grandparent','relative'] , function ( k , v ) {
            self.personal_relation_list.push ( properties.P[v] ) ;
        } ) ;
    } ,
    ...
    generateRelations : function ( q ) {
        ...
        $.each ( [me.wd.items[q]] , function ( dummy , item ) {
            var cq = item.getID() ;
            if ( item.hasClaimItemLink ( properties.P.sex , properties.Q.male ) ) item.gender = 'M' ;
            else if ( item.hasClaimItemLink ( properties.P.sex , properties.Q.female ) ) item.gender = 'F' ;
            else if ( item.hasClaimItemLink ( properties.P.entity_type , properties.Q.person ) ) item.gender = '?' ;
        ...
        $.each ( rel[q] , function ( p , ql ) {
            var section ;
            if ( p == properties.P.father || p == properties.P.mother) section = 'parents' ;
            else if ( p == properties.P.brother || p == properties.P.sister ) section = 'siblings' ;
            else if ( p == properties.P.child ) section = 'children' ;
            else section = 'other' ;
        ...
        $.each ( rel , function ( cq , props ) {
            ...
            $.each ( props , function ( p , ql ) {
                $.each ( ql , function ( k , v ) {
                    ...
                    if ( p == properties.P.father || p == properties.P.mother) {
                        section = 'children' ;
                        real_p = properties.P.child ;
                    } else if ( p == properties.P.brother || p == properties.P.sister ) {
                        section = 'siblings' ;
                        if ( me.wd.items[cq].gender == 'M' ) real_p = properties.P.brother ;
                        else if ( me.wd.items[cq].gender == 'F' ) real_p = properties.P.sister ;
                        else val = {type:'item',mode:2} ;
                    } else if ( p == properties.P.child ) {
                        section = 'parents' ;
                        if ( me.wd.items[cq].gender == 'M' ) real_p = properties.P.father ;
                        else if ( me.wd.items[cq].gender == 'F' ) real_p = properties.P.mother ;
                        else val = {type:'item',mode:2} ;
                    } else {
                        section = 'other' ;
                        if ( p != properties.P.spouse ) val = {type:'item',mode:2} ;
                    }
                    val.q = cq ;
                    val.key = val.q ;
...
//					if ( val.q === undefined ) return ;
        ...
        $.each ( parents , function ( dummy , par ) {
            ...
            if ( undefined === rel[par][properties.P.child] ) return ;
            $.each ( rel[par][properties.P.child] , function ( k , v ) {
                ...
                else if ( me.wd.items[v.key].gender == 'M' ) real_p = properties.P.brother ;
                else if ( me.wd.items[v.key].gender == 'F' ) real_p = properties.P.sister ;
                else val = {type:'item',mode:2} ;
                val.q = v.key ;
                val.key = val.q ;
                val.qualifiers = $.extend(true,{},v.qualifiers);
