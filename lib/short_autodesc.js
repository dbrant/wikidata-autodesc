...
// LANGUAGE-SPECIFIC DATA AND METHODS
    ...
    isPerson : function ( claims ) {
        return ( this.hasPQ( claims , P.instance_of, Q.human) );
    } ,

    isTaxon : function ( claims ) {
        return ( this.hasPQ( claims , P.instance_of, Q.taxon) );
    } ,

    isFilm : function ( claims ) {
        return ( this.hasPQ( claims , P.instance_of, Q.film) || this.hasPQ( claims , P.instance_of, Q.television_program) || this.hasPQ( claims , P.instance_of, Q.television_series) );
    },
    ...
    describePerson : function ( q , claims , opt ) {
        var self = this ;
        var load_items = [] ;
        self.addItemsFromClaims ( claims , P.occupation , load_items );
        self.addItemsFromClaims ( claims , P.position_held , load_items );
        self.addItemsFromClaims ( claims , P.nationality , load_items );
        self.addItemsFromClaims ( claims , P.award_received , load_items );
        self.addItemsFromClaims ( claims , P.instance_of , load_items );
        self.addItemsFromClaims ( claims , P.father , load_items );
        self.addItemsFromClaims ( claims , P.mother , load_items );
        self.addItemsFromClaims ( claims , P.spouse , load_items );
        self.addItemsFromClaims ( claims , P.member_of , load_items );

        var is_male = self.hasPQ ( claims , P.sex , Q.male ) ;
        var is_female = self.hasPQ ( claims , P.sex , Q.female ) ;

        self.labelItems ( load_items , function ( item_labels ) {
            var h = [] ;

            // Nationality
            var h2 = self.getNationality(item_labels, claims, opt);
            if ( h2 != '' ) h.push ( h2 ) ;

            // Occupation
            var ol = h.length ;
            self.add2desc ( h , item_labels , [ P.occupation ] , { hints:{is_male:is_male,is_female:is_female,occupation:true}, o:opt, max_count:2 } ) ;
            if ( h.length == ol ) h.push ( self.getStockString('person', opt.lang) ) ;

            // Office
            self.add2desc ( h , item_labels , [ P.position_held ] , { hints:{is_male:is_male,is_female:is_female,office:true}, prefix:',', o:opt, max_count:2 } ) ;

            // Dates
            var born = self.getYear ( claims , P.date_of_birth , opt.lang ) ;
            var died = self.getYear ( claims , P.date_of_death , opt.lang ) ;
            if ( born != '' && died != '' ) {
                h.push ( ' (' + born + '–' + died + ')' ) ;
            } else if ( born != '' ) {
                h.push ( ' (' + born + '–)' ) ;
            ...
            //if ( self.hasPQ ( claims , P.sex , Q.female ) ) h.push ( '♀' ) ; // Female
            //if ( self.hasPQ ( claims , P.sex , Q.male ) ) h.push ( '♂' ) ; // Male

            self.add2desc ( h , item_labels , [ P.member_of ] , { prefix:';' , txt_key:'member of',o:opt } ) ;
            //self.add2desc ( h , item_labels , [ P.father, P.mother ] , { prefix:';' , txt_key:'child of',o:opt } ) ;
            //self.add2desc ( h , item_labels , [ P.spouse ] , { prefix:';' , txt_key:'spouse of',o:opt } ) ;

            if ( h.length == 0 ) h.push ( self.getStockString('person', opt.lang) ) ;
    ...
    describeTaxon : function ( q , claims , opt ) {
        ...
        self.addItemsFromClaims ( claims , P.taxon_rank , load_items );
        self.addItemsFromClaims ( claims , P.parent_taxon , load_items );
        ...
        self.labelItems ( load_items , function ( item_labels ) {
            ...
            self.add2desc ( h , item_labels , [ P.subclass_of, P.instance_of, P.is_a, P.taxon_rank ] , { o:opt } ) ;
            self.add2desc ( h , item_labels , [ P.parent_taxon ] , { prefix:'' , txt_key:'of',o:opt } ) ;

            if ( h.length == 0 ) h.push ( self.getStockString('species', opt.lang) ) ;
            self.setTarget ( opt ,  self.ucFirst ( h.join ( ' ' ) ) , q ) ;
        } , opt ) ;
    } ,

    describeFilm : function ( q , claims , opt ) {
        var self = this ;
        var load_items = [] ;
        self.addItemsFromClaims ( claims , P.instance_of , load_items );
        self.addItemsFromClaims ( claims , P.publication_date , load_items );
        self.addItemsFromClaims ( claims , P.director , load_items );
        self.addItemsFromClaims ( claims , P.producer , load_items );
        self.addItemsFromClaims ( claims , P.genre , load_items );
        self.addItemsFromClaims ( claims , P.country_of_origin , load_items );
        self.addItemsFromClaims ( claims , P.cast_member , load_items );

        self.labelItems ( load_items , function ( item_labels ) {
            var h = [];
            var h2;

            var pubdate = self.getYear ( claims , P.publication_date , opt.lang ) ;
            if ( pubdate != '' ) h.push ( pubdate ) ;

            h2 = self.getCountryOfOrigin(item_labels, claims, opt);
            if ( h2 != '' ) h.push ( h2 ) ;

            if (self.hasP(claims, P.genre) && self.hasPQ( claims , P.instance_of, Q.film)) {
                self.add2desc ( h , item_labels , [ P.genre ] , { o:opt, max_count:1 } ) ;
            } else {
                self.add2desc ( h , item_labels , [ P.instance_of ] , { o:opt, max_count:1 } ) ;
            }

            self.add2desc ( h , item_labels , [ P.director ] , { txt_key:'directed by', o:opt } ) ;
            self.add2desc ( h , item_labels , [ P.cast_member ] , { prefix:',', txt_key:'starring', o:opt, max_count:2 } ) ;

            // Fallback
            if ( h.length == 0 ) h.push ( self.getStockString('film', opt.lang) ) ;
            self.setTarget ( opt ,  self.ucFirst ( h.join ( ' ' ) ) , q ) ;
        }, opt ) ;
    },
    ...
    describeGeneric : function ( q , claims , opt ) {
        ...
        self.labelItems ( load_items , function ( item_labels ) {
            ...
            // Fallback
            if ( h.length == 0 ) {
                h = '<i>'+self.getStockString('cannot_describe', opt.lang)+'</i>' ;
    ...
    loadItem : function ( q , opt ) {
        ...
        self.wd.getItemBatch ( [ q ] , function () {
            ...
            else if ( self.isFilm ( claims ) ) self.describeFilm ( q , claims , opt ) ;
            else if ( self.isDisambig ( claims ) ) self.setTarget ( opt , self.getStockString('disambig', opt.lang) , q ) ;
