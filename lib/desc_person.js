...
function describe ( ad, claims, opt, callback ) {
    ...
    var load_items = [] ;
    ad.addItemsFromClaims ( claims , [ P.occupation, P.position_held, P.nationality, P.award_received, P.instance_of, P.father, P.mother, P.spouse, P.member_of ] , load_items );
    ...
    ad.labelItems ( load_items , function () {
        ...
        // Occupation
        var ol = h.length ;
        ad.add2desc ( h , claims , [ P.occupation ] , { hints:{is_male:is_male,is_female:is_female,occupation:true}, o:opt, max_count:2 } ) ;
        if ( h.length == ol ) h.push ( ad.getStockString('person', opt.lang) ) ;

        var presidentClaim = ad.getPQ(claims, P.position_held, 11696);
        if (presidentClaim !== null) {
            var val = ad.getQualifierValue(presidentClaim, P.series_ordinal);
            if (val !== null) {
        ...
        // Office
        ad.add2desc ( h , claims , [ P.position_held ] , { hints:{is_male:is_male,is_female:is_female,office:true}, prefix:',', o:opt, max_count:2 } ) ;
